from pyhdf.SD import SD, SDC
import numpy as np

FILE_PATH = "data/MOD06_L2.A2021196.0000.061.2021269014955.hdf"

def load_cloud_water_path():
    hdf = SD(FILE_PATH, SDC.READ)

    # Liquid Water Path (g/m^2)
    ds = hdf.select("Cloud_Water_Path")
    raw = ds[:].astype(np.float32)

    attrs = ds.attributes()

    fill = attrs["_FillValue"]
    valid_min, valid_max = attrs["valid_range"]
    scale = attrs["scale_factor"]
    offset = attrs["add_offset"]

    # mask invalid
    raw[raw == fill] = np.nan
    raw[(raw < valid_min) | (raw > valid_max)] = np.nan

    # apply scaling
    cwp = raw * scale + offset

    print("Cloud Water Path loaded ✅")
    print("CWP shape:", cwp.shape)
    print("CWP min/max:", np.nanmin(cwp), np.nanmax(cwp))

    return cwp


if __name__ == "__main__":
    load_cloud_water_path()
